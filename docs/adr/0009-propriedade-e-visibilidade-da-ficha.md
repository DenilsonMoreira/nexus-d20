...
Criação e atualização são auditadas na mesma transação, com estado anterior e posterior. Edições executadas pelo mestre são reversíveis somente quando a ficha atual ainda corresponde integralmente ao estado posterior registrado. A API calcula os modificadores a partir das seis pontuações usando `floor((pontuação - 10) / 2)`.
...
- Uma alteração posterior bloqueia a reversão de um evento anterior, evitando sobrescrever trabalho mais recente.
